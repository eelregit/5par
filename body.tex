...
\paulo{How about: ``Gompertz-polynomial-shaped reionization can reproduce standard CMB analyses. Now, we move to establish...''?} Having confirmed that the Gompertz-polynomial-shaped reionization can
...
We employ \texttt{PySR}, an SR package,\PMCtodo{some of the material 
now appears earlier} to establish the cosmological dependence of the 
rescaling in \cref{eq:map}.
...
requiring only five cosmological parameters, $\boldsymbol{\theta} = 
\{ \sigma_8, \ns, h, \Omegab, \Omegam\}$.
...
cosmology.}
\paulo{How about now?}
\YL{Can we ``average'' the font sizes of the legend and other texts?} \paulo{Better now?}
...
\paulo{I didn't do this initially since it will convolute the gomp vs gomp.5,
but let me see how bad it gets.}}
\YLtodo[inline]{Would it be useful to add a (much narrower) band around
gomp curves too?}
\label{fig:history}
\end{figure}
