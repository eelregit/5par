...
necessitating the \YL{additional empirical parameter} $\tau_\reio$ in CMB
analyses.
...
simultaneously tightening constraints in cosmological \YL{and astrophysical}
parameters.
...
\YL{related via a power-law transform}, and derive through SR \YL{the
...
We integrate this \YL{universally shaped reionization timeline} into
...
intermediate transitions of fast parameters. \YL{Can we remove this
cobaya advertisement which also collides with the citation superscripts?}}, showcasing its ability to
...
We construct \YL{the} universal $x_\HI$ shape using \YL{256} Sobol samples
of \texttt{21cmFAST} simulations\YL{, varying 5 cosmological parameters
...
All $x_\HI(a)$ profiles share \YL{the same} shape, with differences between
scenarios being mere translations and rescalings \YL{in logarithmic scale
factor $\ln a$}.
...
reionization simulations \cite{Trac2018, Doussot2019} \YL{and favored by
astrophysical data (see \Cref{fig:history}).}
\YL{In contrast, the SR-inspired} Gompertz curve, an asymmetric sigmoid
...
Before fully leveraging our \YL{methodology} to extract the \YL{parameter}
...
\YL{(while fixing ...?)},
while for $\tanh$, the reionization midpoint $z_\re$ is the tuning
parameter.
...
intervals.
\YL{(0.01? Also I am not sure about the interpretation or GR here)}
...
results \cite{Planck2020a}, with \YL{differences} $\lessapprox 0.5 \%$.

\YL{Having demonstrated that the universally shaped reionization can
...
This rescaling naturally depends on cosmology \YL{and astrophysics}.
...
\cref{eq:map,eq:SR_a,eq:SR_b} \YL{are analytic expressions and thus
interpretable.
...
Note that the role of $\sigma_8$ and $\ns$ in $\beta$ slightly reduces
the impact of these trends.
\YL{(what trends? last 2 sentence for what purpose?)}
...
out.
\YL{(increasing $h$ also increases physical baryon density)}
...
over $\zetaUV$.
\YL{(sounds strange to mention marginalization before constraining)}
...
\cite{Greig2017}.
\YL{(need be careful because direct constraining power from DW)}
...
effectively \YL{constrain and} marginalize over $\zetaUV$.
...
wing data on $x_\HI$ to \YL{constrain and} marginalize over reionization
astrophysics.
...
history $x_\HI(z)$.}
\YL{(Use green dotted for SRFull, on top of, purple dashed SRHalf.
I tried to do this myself but history.py confusingly require some
tau files not in the repo, nor can I find the code to generate them)}
Our best-fit gomp + SRFull model (green dashed line) of Planck CMB data
+ quasar damping wing (DW) data is asymmetric and differs significantly
from that of the symmetric $\tanh$ model (blue lines, with dotted region
representing the 1$\sigma$ errors).
...
0.95) \approx 560 $ Myr, \YL{consistent with our previous constraint
without SR.}
...
our best-fit values\YL{, in contrast with the poor fit of symmetric $\tanh$
...
We note that our results assume a flat Universe. Relaxing this assumption
requires adding an extra parameter to the functional form of 
$\tau$\cite{Anselmi2023}. Furthermore, our results are influenced by the semi-numerical
...
analyses. \YL{(what choice? maybe mention the bigger param space we are working on?)}
...
Throughout this work we considered only DW \YL{for astrophysical} data,
