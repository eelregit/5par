...
It was immediatly obviously to us that the $x_\HI$'s had a common shape
...
We fit the composed shape to minimize the mean squared error
(chi-squared with unit covariance) in 128 $x_\HI$'s and at 92 time
points in each, and find the objective value improve with $m$ but only
marginally from $P_5$ to $P_7$.
...
\cite{Cranmer2020b}.
\YLtodo{Miles, it'd be great if you can refine/rewrite the SR and PySR
intro}
...
Once again, we find a constant tilt $\tilt = 8.331$, remarkably close to
the value obtained using the full simulated data (8.290).
