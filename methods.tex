...
\begin{table}[t]
\centering
\caption{\textbf{Parameter constraints from Planck CMB and astrophysical (DW) data.}
...
comparison.
The validation models are gomp and $\tanh$, gomp does not include
the cosmological information present in the timeline of reionization.
In contrast, gomp + SRFull, and the alternative mapping gomp + SRHalf, both include
...
factors of 5 and 2.3 on $\tau_\reio$ and $A_s$, respectively. We also
highlight our extremely competitive $\zetaUV$ constraint.
...
$\tau_\reio$ & \sampled 0.0544(73) & \sampled 0.0543(75) & \sampled 0.0547(76) & \textbf{0.0526(15)} & $\boldsymbol{0.0527^{(14)}_{(16)}}$ & $0.05720_{(77)}^{(88)}$ \\
...
\caption{\textbf{Analysis of CMB data and astrophysical data treating $\tau_\reio$ as a derived
...
but we use the astrophysical data to marginalize over reionization astrophysics.
...
scales implying we are safe from overfitting and supported by
...
CMB jointly with astrophysical data in \Cref{tab:uber-table}.
Comparing these findings to our previous results, we observe strikingly
similar values for all cosmological parameters, albeit with marginally smaller
...
enhances CMB analyses. \Cref{fig:history} includes upper limits from dark
...
emitters\cite{Mason2018, Mason2019, Hoag2019} (high-$z$ galaxies) and
...
constraints on $x_\HI$ (green pentagons in \Cref{fig:history}), reflecting
...
since its inclusion already can halve the error on $\tau_\reio$ and because more
data will enhance our ability to marginalize over reionization astrophysics.
...
jointly with CMB to map reionization to cosmological parameters by
...
reionization for gomp + SRFull + CMB + DW nor gomp + SRHalf + CMB + DW models,
