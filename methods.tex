...
parameters, we conducted \YL{256} \texttt{21cmFASTv3} simulations to generate
...
We \YL{first} use a scrambled Sobol sequence \cite{Sobol1967, Owen1998} \YL{of
...
\YL{Because most of the 6D hypercube volume lies near its surface, we name
...
\caption{\textbf{Sobol \YL{design for our $x_\HI$ edge and core samples}},
...
of the parameter space within the \YL{edge} prior range of \cref{eq:prior}\YL{,
...
\cite{Park2019} or redshift\YL{, which we will investigate in a future
work.}
...
regression, to model their \YL{dependences on the independent cosmological
...
1.2\times10^{-3}$ ($\sim 0.03\%$ error on average) \paulo{Update numbers}, while higher
complexities can help but very slowly.
\YL{(I got $\lesssim 0.05\%$ as the relative error, by dividing by the
complexity 1 constant. that what you meant?)}
...
mapping between \YL{physical parameters} and $x_\HI$ profiles.
...
cosmological \YL{and astrophysical dependences} present in the timeline
of reionization.
In contrast, gomp + SRFull, and the alternative mapping gomp + SRHalf,
both include the additional \YL{parameter dependences} in the reionization
timeline.
...
\YL{constrain and} marginalize over reionization astrophysics.
...
Note that we have not use our mapping from \YL{physical parameters} to
$\tau_\reio$.
...
reproducing the simulated \YL{$x_\HI$(?)} (edge + core) with SRHalf.
...
\YL{(I'm a bit confused what this comparasion with Planck is here)}
...
more data becomes available, particularly since \YL{we find including them
could} already halve the error on $\tau_\reio$, and because more data
will enhance our ability to \YL{constrain and} marginalize over reionization
astrophysics.
\YL{We opt not to include the LF constraints on $x_\HI$ in this work as they
...
jointly with CMB \YL{to infer cosmology while constraining and
marginalizing over $\zetaUV$.}

\YL{(Breaking below to a new paragraph)}
...
$\beta$ to the mean value (\YL{7.66, the complexity 1 expression from}
\texttt{PySR}) and vary $\alpha$.
Therefore, we do not use the \YL{SR} mapping from \YL{physical parameters to}
...
Instead, we use SRFull or SRHalf to map \YL{physical parameters} to
reionization timeline and include the \YL{astrophysical} likelihood
component.
...
\YL{FIXME: rearrange floats and barriers at the end}
\FloatBarrier
