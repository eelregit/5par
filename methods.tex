...
%\begin{sidewaystable}
\centering
\caption{\textbf{Parameter constraints from Planck CMB and DW data.}
...
In this work, we supplement Planck CMB data with constraints on $x_\HI$ 
by quasar damping wings (DW) (see green pentagons in \Cref{fig:history}). 
Note that this decision reflects our expectation regarding the robustness of 
the constraints. An interesting future exercise would be to include luminosity 
function constraints when jointly with DW as more data becomes available. 
...
is calculated in \texttt{Cobaya} based on the used astrophysical dataset.  
...
reionization in gomp + SRFull + CMB + DW nor gomp + SRHalf + CMB + DW runs, 
...
for models that take advantage of \cref{eq:premise}.
