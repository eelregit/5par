...
\end{align}
%
The distribution of our simulations is displayed in \autoref{fig:sobol}.
...
Our 21cmFAST simulations have a 300 comoving Mpc box side and $768^3$
...
\paulo{Do we need more info about the rescale? If so, Yin feel free to
add text to this subsection or to modify existing text in the
caption/body of the paper.}

\begin{figure}
\centering
\includegraphics{figs/shape_6.pdf}
...
\subsection*{PySR alternative mapping}
\label{ssec:0226}
...
Similar to Table \ref{tab:tau_comp} but including the cosmological
mapping to reionization histories.
...
instead, it is obtained directly with Eq.~(\ref{eq:uni}).
Furthermore, the $\tanh$ model included here samples over $z_\re$
instead of $\tau_\reio$.}
\label{tab:para}
\end{table}

Applying Eq.~(\ref{eq:SR0226}) we reanalyze the CMB data following the
