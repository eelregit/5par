...
def gen_history(i):
    sigma_8, n_s, h, Omega_b, Omega_m, zeta_eff, T_vir, E0, L_X = Sobol_table[i]
    ...
    with p21c.global_params.use(
        ...
        lightcone = p21c.run_lightcone(
            redshift = 2., # minimum redshift, choose 5 to compare with Catalina's
            ...
            cosmo_params = p21c.CosmoParams(
                ...
                OMb=Omega_b,
                POWER_INDEX=n_s),
            astro_params = {'R_BUBBLE_MAX':50, 'L_X':L_X, "HII_EFF_FACTOR":zeta_eff, "NU_X_THRESH":E0, "ION_Tvir_MIN":T_vir},
    ...
    T_vir_array = np.ones(len(z))
    E0_array = np.ones(len(z))
    L_X_array = np.ones(len(z))
    ...
    T_vir_array = T_vir_array * lightcone.astro_params.ION_Tvir_MIN
    E0_array = E0_array * lightcone.astro_params.NU_X_THRESH
    L_X_array = L_X_array * lightcone.astro_params.L_X
    ...
    np.savetxt(
        ...
        np.stack([z_array, ave_xH, sigma8_array, ns_array, hlittle_array, OMb_array,
            OMm_array, zeta_eff_array, T_vir_array, E0_array, L_X_array], axis=0),
        delimiter=' ',
        fmt='%e')
