...
\title{Supplementary Material \\ \Large Five parameters are all you need\\(in $\Lambda$CDM)\vspace{2em}}
...
To establish the universality of our proposed Gompertzian model for the
...
\end{alignat}
%
Their 1D and 2D projections in Fig.~\ref{fig:sobol} illustrate the sample
...
of the parameter space within the edge prior range of Eq.~(\ref{eq:prior}),
...
contribution of free electrons due to the Gompertzian contribution -- Eq.~(1) of the main body -- and $x^\rec_\e
...
composition of the Gompertzian curve with a 5th-degree polynomial in Eqs.~(1) and (2) of the main article.
...
\emph{(Bottom)} Timelines transformed by the inverse of Gompertzian
function, modeled in blue curve with a 5th-degree polynomial in
Eq.~(2) of the main article.}
\label{fig:shape}
\end{figure}
...
However, given our broad parameter range in Eq.~(\ref{eq:prior}), some
...
$x_\HI$ timeline.
We compose the Gompertz function $\gomp$, defined in Eq.~(1) of the main article, with
...
(see the lower panel of Fig.~\ref{fig:shape}), and has 6 parameters to fit.
...
a power law in Eq.~(3) of the main article.
...
Fig.~\ref{fig:shape} shows all 256 $x_\HI$ timelines and their universality
...
same analytic shape in Eqs.~(1) and (2) of the main article, with the same coefficients
...
In Fig.~\ref{fig:thesan}, we show that our universal shape accurately fits
...
complexity 22 point gives Eq.~(4) in the main article.}
\label{fig:pareto}
\end{figure}
...
parameters in Fig.~\ref{fig:sobol} as inputs and output $\ln\ap$ or $\tilt$,
...
typically visualized by the Pareto front, as shown in Fig.~\ref{fig:pareto}.
...
Unsurprisingly, Eqs.~(\ref{eq:SRHalf_a}) and (\ref{eq:SRHalf_b}) recover the cosmological
...
Furthermore, Eq.~(\ref{eq:SRHalf_b}) excludes $\ns$ and $\sigma_8$ because of
...
Replacing Eqs~(4) and (5) of the main article with Eqs.~(\ref{eq:SRHalf_a}) and (\ref{eq:SRHalf_b}), we
...
Fig. 4 in the main article includes upper limits from dark pixel
...
(DW) constraints on $x_\HI$ (green pentagons in Fig. 4 of the main article),
...
reionization -- Eqs.~(4) and (5) of the main article or alternatively
Eqs.~(\ref{eq:SRHalf_a}) and (\ref{eq:SRHalf_b}).
...
Future work will explore the implications of Gompertzian reionization for
...
Eqs.~(1) to (5) in the main article vs.\ sampling it with the
conventional $\tanh$ model.}
Here gomp + SRFull combines our Gompertzian universal shape with the
rescaling pivot and tilt obtained via symbolic regression.
Note that for gomp + SRFull we do not sample over $z_\re$, since
Eq.~(1) of the main article does not depend on it but we use the astrophysical data to
...
our Gompertzian universal shape ($\tanh$ model).}
...
in Eqs.~(1) and (2) of the main article vs.\ the conventional $\tanh$ model using CMB
